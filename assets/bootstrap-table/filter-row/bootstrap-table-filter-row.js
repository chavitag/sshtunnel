    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            ...
            select: function (that, field, isVisible) {
                ...
                return sprintf('<div class="input-group selectpicker col-md-12 multiple">'+select+'<span class="input-group-append"><button class="btn btn-outline-secondary border-left-0 border" type="button" style="padding-top:2px; padding-bottom: 2px"><i class="fa fa-trash"></i></button></span></div>',
        ...
        if (this.options.filterControl && this.options.filterShowClear) {
            ...
            if (!$btnClear.length) {
               ...
               $btnClear.off('click').on('click', $.proxy(this.clearFilterControl, this));
				...
				if (!$btnOpen.length) {
                $btnOpen = $([
                    sprintf('<button class="btn btn-%s filter-show-open" ', this.options.buttonsClass),
                    sprintf('type="button" title="%s">', this.options.formatOpenFilters()),
                    sprintf('<i class="%s %s"></i> ', this.options.iconsPrefix, this.options.icons.open),
                    '</button>'
                ].join('')).appendTo($btnGroup);
